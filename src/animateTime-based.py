...
# === Multi-Robot Animation with Enhanced Collision Detection & Timeline ===
...
def detect_collisions(robot_specs, positions, speed_mps, delta_t):
    """
    Detect multi-robot collisions via pure interval overlap per node.
    Returns list of ([robot_ids], node, collision_time).
    """
    # Precompute arrival/departure intervals
    arrivals_list = [compute_node_times(spec['path'], positions, speed_mps)
    ...
    for rid, spec in enumerate(robot_specs, start=1):
        ...
        specs_times.append((rid, path, arrivals, departures))

    # Aggregate occupancy intervals by node
    node_occ = {}
    ...
    for node, occs in node_occ.items():
        n = len(occs)
        # check all combinations of size >=2
        for k in range(n, 1, -1):
            for combo in combinations(occs, k):
                present = {rid for rid, _, _ in combo}
                if len(present) < 2:
                    continue
                ...
                if max(starts) <= min(ends):
                    ...
                    key = (tuple(sorted(present)), node, round(collision_time, 2))
                    if key not in seen:
                        seen.add(key)
                        collisions.append((sorted(present), node, collision_time))
            # continue checking smaller groups to catch all collisions
    return collisions
...
def animate_robots(robot_specs, positions, graph,
                   speed_mps=1.0, fps=30, topo_map=None,
                   collision_delta_t=2.0):
    # Print paths
    for rid, spec in enumerate(robot_specs, start=1):
    ...
    for robots, node, t in collisions:
        print(f"[Collision] Robots {','.join(map(str,robots))} at node '{node}': {t:.2f}s")
...
if __name__ == '__main__':
    ...
    multi_robot_goals = [
        ['Park1', 'Start1', 'T00', 'T01', 'T02', 'T03', 'Spare1', 'Park1'],
        ['Park2', 'Start1', 'T10', 'Spare1', 'Park2'],
        ['Park3', 'Start2', 'T40', 'T41', 'T42', 'T43', 'Spare2', 'Park3']
    ]
