...
# def _interval_conflicts(intervals: List[Tuple[float, float, str]],
#                         start: float, end: float, gap: float) -> bool:
#     if not intervals:
#         return False
#     s_q = start - gap
#     e_q = end + gap
#     bare = [(s, e) for (s, e, _) in intervals]
#     i = bisect_right(bare, (s_q, e_q))
#     if i > 0:
#         s, e = bare[i - 1]
#         if not (e <= s_q or e_q <= s):
#             return True
#     if i < len(bare):
#         s, e = bare[i]
#         if not (e <= s_q or e_q <= s):
...
# def _next_free(intervals: List[Tuple[float, float, str]], t: float, gap: float) -> float:
#     if not intervals:
#         return t
...
def _next_free_node_excluding(intervals: List[Tuple[float, float, str]],
                              t: float, gap: float, who: Optional[str]) -> float:
    if not intervals:
        return t
    intervals = sorted(intervals, key=lambda x: (x[0], x[1]))
    cur = t
...
#     return "∞" if not math.isfinite(val) else f"{val:.2f}s"

# def _fmt_delta(val: float, now: float) -> str:
#     return "∞" if not math.isfinite(val) else f"{(val - now):.2f}s"
...
class ReservationTable:
    ...
    # ---- query
    # def has_conflict_edge(self, u: str, v: str, t0: float, t1: float) -> bool:
    #     return _interval_conflicts(self.edge_res.get((u, v), []), t0, t1, self.gap)

    # def has_conflict_node(self, node: str, t0: float, t1: float) -> bool:
    #     return _interval_conflicts(self.node_res.get(node, []), t0, t1, self.gap)

    # def node_occupied_by_other(self, node: str, who: str) -> bool:
    #     """True if there's any open-ended occupancy on 'node' by someone else."""
    #     return any(name != who for (_, name) in self.open_nodes.get(node, []))

# ==========================
# Earliest feasible depart
# ==========================

def earliest_depart(G: nx.Graph, res: ReservationTable,
                    u: str, v: str, t: float, w: float, who: str) -> float:
    """
...
# def _wait_vs_detour(G: nx.Graph, res: ReservationTable, now_t: float,
#                     current: str, goal: str, candidate_nodes: List[str],
#                     who: str, lookahead_edges: int = 1) -> List[str]:
#     # Identify the first conflicted hop (or treat first arrival open-occupied as conflict)
#     if len(candidate_nodes) >= 2:
#         first_u, first_v = candidate_nodes[0], candidate_nodes[1]
#         if res.node_occupied_by_other(first_v, who):
#             first_conflict = (first_u, first_v)
#         else:
#             _, _, first_conflict = _simulate_prefix_eta(candidate_nodes, now_t, G, res, who, edges=None)
#     else:
#         _, _, first_conflict = _simulate_prefix_eta(candidate_nodes, now_t, G, res, who, edges=None)

#     # Full ETA along candidate
#     eta_A, waited_A, _ = _simulate_prefix_eta(candidate_nodes, now_t, G, res, who, edges=None)

#     # If no conflict -> stick with candidate
#     if first_conflict is None:
#         return candidate_nodes

#     # Build detour graph by removing conflicted node (preferred) or edge
#     u, v = first_conflict
#     H = G.copy()
#     removed = False
#     if v not in (current, goal) and H.has_node(v):
#         H.remove_node(v); removed = True
#     elif H.has_edge(u, v):
#         H.remove_edge(u, v); removed = True

#     detour = None
#     if removed:
#         try:
#             detour = nx.shortest_path(H, source=current, target=goal, weight="weight")
#         except nx.NetworkXNoPath:
#             detour = None

#     # Compute detour ETA (if exists)
#     if detour is not None:
#         eta_B, _, _ = _simulate_prefix_eta(detour, now_t, G, res, who, edges=None)
#     else:
#         eta_B = float("inf")

#     return detour if (detour is not None and (eta_B + 1e-9 < eta_A or not math.isfinite(eta_A))) else candidate_nodes
